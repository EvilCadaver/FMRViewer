...
def find_thetas(H = Ho, Hk = Hk, Ms = Ms, phi = phi):
    ...
    eps = 1e-10
    true_thetas = [theta for theta in thetas if abs(A*np.sin(theta)*np.cos(theta)-B/2+B*np.sin(theta)**2+H*np.sin(theta))<eps]
    ...
    print('H original check:', [Hk/2*np.sin(2*(phi-theta))/np.sin(theta) - 4*np.pi*Ms*np.cos(theta) for theta in theta2])
    return true_thetas
